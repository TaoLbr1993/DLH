...
int main(int argc, char **argv) {
    ...
    // 在构建 PSL 之前设置 BloomFilter 的 FPP
    BFLabelHashEle::false_positive_probability = bf_fpp;

    // 构建 PSL（基于超级节点图的边）
    DisOracle psl_oracle(grs.edge_pairs, ds.meta.k_hop_max, false);
    psl_oracle.see_labels(); // 与示例保持一致

    std::cout << "BFLabelHashEle::false_positive_probability = " << std::fixed << std::setprecision(3)
        << BFLabelHashEle::false_positive_probability << std::endl;
    ...
    for (size_t ef : efs) {
        ...
        std::cout << "[DLH-M] ef=" << ef << " recall=" << std::fixed
    ...
    oss << "\nDLH-M: " << std::fixed << std::setprecision(3);
    ...
    std::ofstream fout(out_dir + "/DLH-M_stats.log", std::ios::out | std::ios::trunc);
    fout << oss.str();
    std::cout << "日志写入: " << (out_dir + "/DLH-M_stats.log") << std::endl;
