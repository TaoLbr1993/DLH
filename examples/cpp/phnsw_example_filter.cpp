...
int main() {
    ...
    hnswlib::PT2DHNSW<float>* alg_hnsw = new hnswlib::PT2DHNSW<float>(&space, max_elements, M, ef_construction);

    // NEW
    // init point relations
    hnswlib::Point2DSampler pt2ds = hnswlib::Point2DSampler(0,1,0,1);
    alg_hnsw->setRange(&pt2ds, 0.2);
