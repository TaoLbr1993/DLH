...
int main() {
    ...
    int M = 16;                 // Tightly connected with internal dimensionality of the data
                                // strongly affects the smemory consumption
    int ef_construction = 200;  // Controls index search speed/build speed tradeoff

    // Initing index
    hnswlib::L2Space space(dim);
    hnswlib::GraphHNSW<float>* alg_hnsw = new hnswlib::GraphHNSW<float>(&space, max_elements, M, ef_construction);

    // NEW
    // init graph relations
    hnswlib::GraphRelationSampler grs = hnswlib::GraphRelationSampler(0.1);
    alg_hnsw->setGraphHop(&grs, 2);
    ...
    for (int i = 0; i < max_elements; i++) {
        // NEW
        alg_hnsw->addPointLimit(data + i * dim, i);
    }
