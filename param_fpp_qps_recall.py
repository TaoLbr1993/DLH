...
# ----------------------------
# 1) 硬编码：需要读取哪些 logs 文件夹（不同 fpp）
#    目录名需包含类似：Deep10M-0.00027-2M-... 这种片段，脚本会解析中间的 fpp=0.00027
# ----------------------------
LOG_DIRS: List[str] = [
    # 按你的实际目录修改（示例占位）：
    "logs/Sift1M-0.00027-1M-4hop-20group-fpp0.001",
    "logs/Sift1M-0.00027-1M-4hop-20group-fpp0.005",
    "logs/Sift1M-0.00027-1M-4hop-20group-fpp0.01",
    "logs/Sift1M-0.00027-1M-4hop-20group-fpp0.015",
]
...
OUT_DIR = Path("figures/param_fpp")
OUT_LEGEND = OUT_DIR / "param_fpp_legend.svg"
OUT_QPS = OUT_DIR / "param_fpp_qps.svg"
OUT_RECALL = OUT_DIR / "param_fpp_recall.svg"
...
def _parse_fpp(folder_name: str) -> float:
    """
    从目录名解析 fpp。

    你的目录名样例：
      - Sift1M-0.000027-1M-4hop-20group-fpp0.001  -> fpp = 0.001
      - Sift1M-0.000027-1M-4hop-20group           -> 默认 fpp = 0.01

    规则：
      1) 若包含 'fpp' 后缀（如 -fpp0.005），则解析该值；
      2) 否则返回默认值 0.01。
    """
    m = re.search(r"(?:^|[-_])fpp\s*([0-9]+(?:\.[0-9]+)?)\s*(?:$|[-_])", folder_name, flags=re.I)
    if m:
        return float(m.group(1))

    # 不带 fpp 后缀的目录，按你的约定使用默认值
    return 0.01
...
def _collect_series(repo_root: Path) -> Dict[str, Dict[int, List[Tuple[float, float, float]]]]:
    """
    return:
      data[method_key][ef] = list of (fpp, recall, qps), sorted by fpp
    """
    ...
    for d in LOG_DIRS:
        ...
        for m in METHODS:
            ...
            for ef in EFS:
                ...
                data[m["key"]][ef].append((fpp, recall, qps))
...
def _save_qps_svg(data) -> None:
    ...
    # x 轴：保持线性，但用科学计数法显示，避免 0.000xx 挤在一起
    ax.ticklabel_format(axis="x", style="sci", scilimits=(-3, 3), useMathText=True)

    ax.grid(True, linestyle="--", alpha=GRID_ALPHA)

    # x ticks：如果数据点数量不多，直接用数据点；否则让 matplotlib 自己选
