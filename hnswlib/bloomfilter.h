...
public:
   ...
   double estCard() {
        // guarantee the same seed
        size_t cnt = 0;
        size_t length = table_size_/bits_per_char;
        for (std::size_t i = 0; i < length; i++){
            auto bits = bit_table_[i];
            while (bits>0) {
                cnt ++;
                bits = bits & (bits-1);
            }
        }  
        double res = -(((double)table_size_)/salt_.size())*log(1-((double)cnt)/table_size_);
        return res;
   }
