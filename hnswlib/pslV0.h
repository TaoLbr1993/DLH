...
bool DisOracle::query_by_bp_es(int u, int v, int search_k) {
	...
	for (int i = 0; i < N_ROOTS; ++i) {
		...
		// // V1: directly compute
		// if (td-2 <= search_k & (idx_u.bpspt_s[i][0] & idx_v.bpspt_s[i][0])) return true;
		// if (td-1 <= search_k & (idx_u.bpspt_s[i][0] & idx_v.bpspt_s[i][1]) | (idx_u.bpspt_s[i][1] & idx_v.bpspt_s[i][0])) return true;
